use crate::contexts::*;



use yew::context::ContextHandle;
use yew::prelude::*;
use yew::Properties;


use crate::glue::copy_to_clipboard;

use log::{info, error};

#[derive(PartialEq, Properties)]
pub struct Props;

pub struct InputOutputBox {
    input_node_ref: NodeRef,
    output_node_ref: NodeRef,
    text_input: TextInputContext,
    processor_config_names: ProcessorConfigNamesContext,
    _text_input_context_handle: ContextHandle<TextInputContext>,
    _processor_config_names_context_handle: ContextHandle<ProcessorConfigNamesContext>,
}

pub enum Msg {
    Change,
    CopyRaw,
    CopyProcessed,
    Copied(String),
    TextInputContextUpdated(TextInputContext),
    ProcessorConfigNamesContextUpdated(ProcessorConfigNamesContext),
}

impl Component for InputOutputBox {
    type Message = Msg;
    type Properties = Props;

    fn create(ctx: &Context<Self>) -> Self {

        let (text_input, text_input_ctx_handle) = 
        ctx
        .link()
        .context(
            ctx
            .link()
            .callback(Msg::TextInputContextUpdated)
        )
        .expect("Couldn't find a TextInputContext provider.");

        let (processor_config_names, processor_config_names_ctx_handle) = 
        ctx
        .link()
        .context(
            ctx
            .link()
            .callback(Msg::ProcessorConfigNamesContextUpdated)
        )
        .expect("Couldn't find a ProcessorConfigNamesContext provider.");
        // let text = use_context::<TextInput>().expect("Couldn't find a TextInput context");

        Self {
            input_node_ref: NodeRef::default(),
            output_node_ref: NodeRef::default(),
            text_input,
            processor_config_names,
            _text_input_context_handle: text_input_ctx_handle,
            _processor_config_names_context_handle: processor_config_names_ctx_handle
        }
    }

    fn view(&self, ctx: &Context<Self>) -> Html {


        let onkeyup = ctx.link().callback(|_| Msg::Change);

        let on_raw_text_copy_pressed = ctx.link().callback(|_| Msg::CopyRaw);
        let on_processed_text_copy_pressed = ctx.link().callback(|_| Msg::CopyProcessed);

        html! {
            <div class="h-full w-full overflow-auto flex flex-col justify-around">
                <section class="p-4 space-y-4">
                    <div class="w-full flex justify-between items-center">
                        <h1 class="text-2xl"> {"Raw"} </h1>
                        <button class="btn btn-secondary p-0 rounded-md" title="Copy"
                            onclick={on_raw_text_copy_pressed}
                        > 
                            <svg class="h-6 w-12" xmlns="http://www.w3.org/2000/svg" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
                                <g>
                                    <path d="M53.9791489,9.1429005H50.010849c-0.0826988,0-0.1562004,0.0283995-0.2331009,0.0469999V5.0228   C49.7777481,2.253,47.4731483,0,44.6398468,0h-34.422596C7.3839517,0,5.0793519,2.253,5.0793519,5.0228v46.8432999   c0,2.7697983,2.3045998,5.0228004,5.1378999,5.0228004h6.0367002v2.2678986C16.253952,61.8274002,18.4702511,64,21.1954517,64   h32.783699c2.7252007,0,4.9414978-2.1725998,4.9414978-4.8432007V13.9861002   C58.9206467,11.3155003,56.7043495,9.1429005,53.9791489,9.1429005z M7.1110516,51.8661003V5.0228   c0-1.6487999,1.3938999-2.9909999,3.1062002-2.9909999h34.422596c1.7123032,0,3.1062012,1.3422,3.1062012,2.9909999v46.8432999   c0,1.6487999-1.393898,2.9911003-3.1062012,2.9911003h-34.422596C8.5049515,54.8572006,7.1110516,53.5149002,7.1110516,51.8661003z    M56.8888474,59.1567993c0,1.550602-1.3055,2.8115005-2.9096985,2.8115005h-32.783699   c-1.6042004,0-2.9097996-1.2608986-2.9097996-2.8115005v-2.2678986h26.3541946   c2.8333015,0,5.1379013-2.2530022,5.1379013-5.0228004V11.1275997c0.0769005,0.0186005,0.1504021,0.0469999,0.2331009,0.0469999   h3.9682999c1.6041985,0,2.9096985,1.2609005,2.9096985,2.8115005V59.1567993z" />
                                    <path d="M38.6031494,13.2063999H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0158005   c0,0.5615997,0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4542999,1.0158997-1.0158997   C39.6190491,13.6606998,39.16465,13.2063999,38.6031494,13.2063999z" />
                                    <path d="M38.6031494,21.3334007H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0157986   c0,0.5615005,0.4544001,1.0159016,1.0159006,1.0159016h22.3491974c0.5615005,0,1.0158997-0.454401,1.0158997-1.0159016   C39.6190491,21.7877007,39.16465,21.3334007,38.6031494,21.3334007z" />
                                    <path d="M38.6031494,29.4603004H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997   s0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4543991,1.0158997-1.0158997   S39.16465,29.4603004,38.6031494,29.4603004z" />
                                    <path d="M28.4444485,37.5872993H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997   s0.4544001,1.0158997,1.0159006,1.0158997h12.1904964c0.5615025,0,1.0158005-0.4543991,1.0158005-1.0158997   S29.0059509,37.5872993,28.4444485,37.5872993z" />
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                </svg>
                        </button>
                    </div>
                    <textarea
                        ref={self.input_node_ref.clone()}
                        class="textarea textarea-bordered resize-y w-full rounded-lg"
                        rows="12"
                        placeholder="Copy and paste your text here. Change the settings on the right and watch for results on the bottom."
                        onkeyup={onkeyup}
                    >
                    </textarea>
                </section>
                <section class="p-4 space-y-4">
                    <div class="w-full flex justify-between items-center">
                        <h1 class="text-2xl"> {"Processed"} </h1>
                        <button class="btn btn-secondary p-0 rounded-md" title="Copy"
                            onclick={on_processed_text_copy_pressed}
                        > 
                            <svg class="h-6 w-12" xmlns="http://www.w3.org/2000/svg" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
                                <g>
                                    <path d="M53.9791489,9.1429005H50.010849c-0.0826988,0-0.1562004,0.0283995-0.2331009,0.0469999V5.0228   C49.7777481,2.253,47.4731483,0,44.6398468,0h-34.422596C7.3839517,0,5.0793519,2.253,5.0793519,5.0228v46.8432999   c0,2.7697983,2.3045998,5.0228004,5.1378999,5.0228004h6.0367002v2.2678986C16.253952,61.8274002,18.4702511,64,21.1954517,64   h32.783699c2.7252007,0,4.9414978-2.1725998,4.9414978-4.8432007V13.9861002   C58.9206467,11.3155003,56.7043495,9.1429005,53.9791489,9.1429005z M7.1110516,51.8661003V5.0228   c0-1.6487999,1.3938999-2.9909999,3.1062002-2.9909999h34.422596c1.7123032,0,3.1062012,1.3422,3.1062012,2.9909999v46.8432999   c0,1.6487999-1.393898,2.9911003-3.1062012,2.9911003h-34.422596C8.5049515,54.8572006,7.1110516,53.5149002,7.1110516,51.8661003z    M56.8888474,59.1567993c0,1.550602-1.3055,2.8115005-2.9096985,2.8115005h-32.783699   c-1.6042004,0-2.9097996-1.2608986-2.9097996-2.8115005v-2.2678986h26.3541946   c2.8333015,0,5.1379013-2.2530022,5.1379013-5.0228004V11.1275997c0.0769005,0.0186005,0.1504021,0.0469999,0.2331009,0.0469999   h3.9682999c1.6041985,0,2.9096985,1.2609005,2.9096985,2.8115005V59.1567993z" />
                                    <path d="M38.6031494,13.2063999H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0158005   c0,0.5615997,0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4542999,1.0158997-1.0158997   C39.6190491,13.6606998,39.16465,13.2063999,38.6031494,13.2063999z" />
                                    <path d="M38.6031494,21.3334007H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0157986   c0,0.5615005,0.4544001,1.0159016,1.0159006,1.0159016h22.3491974c0.5615005,0,1.0158997-0.454401,1.0158997-1.0159016   C39.6190491,21.7877007,39.16465,21.3334007,38.6031494,21.3334007z" />
                                    <path d="M38.6031494,29.4603004H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997   s0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4543991,1.0158997-1.0158997   S39.16465,29.4603004,38.6031494,29.4603004z" />
                                    <path d="M28.4444485,37.5872993H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997   s0.4544001,1.0158997,1.0159006,1.0158997h12.1904964c0.5615025,0,1.0158005-0.4543991,1.0158005-1.0158997   S29.0059509,37.5872993,28.4444485,37.5872993z" />
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                </svg>
                        </button>
                        </div>
                        <textarea
                            ref={self.output_node_ref.clone()}
                            class="textarea textarea-bordered resize-y w-full rounded-lg disabled:text-gray-200 cursor-pointer"
                            rows="12"
                            placeholder="Change text above and watch this change."
                        >
                        </textarea>
                </section>
            </div>
        }
    }

    fn rendered(&mut self, _ctx: &Context<Self>, _first_render: bool) {
        
        
        if let Some(input) = self.input_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
            input.set_value(self.text_input.raw.as_str());
        }
        if let Some(output) = self.output_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
            output.set_value(self.text_input.raw.as_str());
        }
    }

    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {

        let on_copied = ctx.link().callback(Msg::Copied);

        match msg {
            Msg::TextInputContextUpdated(text_input_context) => {
                info!("TextInputContextUpdated: {:?}", text_input_context.raw.to_string());
                self.text_input = text_input_context;
                if let Some(output) = self.output_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
                    output.set_value(self.text_input.raw.as_str());
                }
            },
            Msg::Change => {
                if let Some(input) = self.input_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
                    let input_text = input.value();
                    self.text_input.dispatch(input_text);
                }
            },
            Msg::CopyRaw => {
                if let Some(input) = self.input_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
                    let input_text = input.value();
                    wasm_bindgen_futures::spawn_local(async move {
                        match copy_to_clipboard(input_text.clone()).await {
                            Ok(_) => {
                                on_copied.emit("SUCCESS".to_string());
                            },
                            Err(_) => {
                                on_copied.emit("FAILED".to_string());
                            }
                        }
                    })
                }
            },
            Msg::CopyProcessed => {
                if let Some(output) = self.output_node_ref.cast::<web_sys::HtmlTextAreaElement>() {
                    let output_text = output.value();
                    wasm_bindgen_futures::spawn_local(async move {
                        match copy_to_clipboard(output_text.clone()).await {
                            Ok(_) => {
                                on_copied.emit("SUCCESS".to_string());
                            },
                            Err(_) => {
                                on_copied.emit("FAILED".to_string());
                            }
                        }
                    })
                }
            }
            Msg::Copied(s) => {
                match s.as_str() {
                    "SUCCESS" => {
                        info!("Copied to clipboard!");
                    },
                    _ => {
                        error!("Failed to copy to clipboard.");
                    }
                }
            }
            Msg::ProcessorConfigNamesContextUpdated(processor_config_names) => {
                self.processor_config_names = processor_config_names;
            }
        }
        true
    }
}